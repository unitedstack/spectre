...
class ModalBase extends React.Component {
  ...
  constructor(props) {
    ...
    'onChangePwd', 'onFocusPwd', 'onBlurPwd', 'getSize',
  ...
  initialize(res) {
    ...
    let size, imageSize = 1;

    if (image && image.size) {
      size = unitConverter(image.size);
      if(!size.unit.indexOf(['B', 'KB', 'MB'])) {
        imageSize = size.num;
      }
    }
  ...
  getSize(image) {
    ...
    if (image && image.size) {
      size = unitConverter(image.size);
      if(['B', 'KB', 'MB'].indexOf(size.unit) === -1) {
        imageSize = size.num;
      }
    }
  ...
  onChangeImageType(key, e) {
    ...
    switch(key) {
      case 'image':
        objImage = image;
        deviceSize = this.getSize(image);
        break;
      case 'snapshot':
        objImage = snapshot;
        deviceSize = this.getSize(snapshot);
        break;
      case 'bootableVolume':
